...
| 11 | **Phase 5.2 单点均值比较忽略方差** | 论文声称值落在点估计附近但 CI 很宽, 判决虚假积极 | 用 95% CI 区间验证, 非点估计对比; 报告 x̄ ± CI 而非单值 |
| 12 | **Phase 6 自动翻译不校对专业术语** | 术语混淆 (如经济学的 "identification" ≠ ML 的 "识别", "general equilibrium" ≠ "一般均衡") | 论文核心术语先在 `analysis/glossary.md` 对齐; 翻译后人工逐条校对 |
| 13 | **Phase 3 基线失败时不记录偏离直接放弃** | 丢失诊断信息, 后续无法追溯失败根因 | 基线失败必须写 `analysis/gray_areas.md` 记录环境/代码/参数偏离; 至少保留完整日志 |
