...
`paperfig deliver` is the canonical acceptance command. It freezes the exact specification bytes into a private same-directory snapshot, renders and checks that snapshot, atomically commits the HTML (and the optional PDF), and reports SHA-256 plus byte counts for specification and artifact(s).
...
`paperfig visual-check <output.html> --json` collects automated browser evidence from the exact delivered HTML without modifying or rerendering it. It opens the HTML at single-column (~336px) and double-column (~672px) widths plus 1440×900 desktop, captures screenshots, and reports scroll overflow checks.
