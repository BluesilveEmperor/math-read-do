...
This stack must work on macOS, Linux, and Windows. Use `pathlib` paths, project-local output directories, and Office-safe fonts or theme fonts. Do not hardcode OS font paths or platform-specific file locations.

Treat LibreOffice/soffice as optional, only when it is already available and a real rendered preview is worth the cost.
...
6. Run the self-review and revision loop.
7. Verify by reopening the PPTX and inspecting package structure.
